...
def config_files(environ=None):
    ...
    try:
        ...
        warnings.warn(
            "The use of the CRESTIC_CONFIG_FILE environment variable is deprecated, please use CRESTIC_CONFIG_PATHS instead.",
            DeprecationWarning
        )
        return retval
    except KeyError:
...
def main(argv, environ=None, conffile=None, dryrun=None, executable=None):
    ...
    if dryrun:
        ...
        try:
            return subprocess.call(argstring, env=restic_environ, cwd=cwd, shell=False)
